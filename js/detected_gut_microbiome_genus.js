// js/detected_gut_microbiome_genus.js
// date: 2025-07-01
// author: Honglie Zhang

const CONFIG = {
  overallDescription: '下列表格展示了狗狗肠道中实际检出的属层级的肠道细菌 (检测丰度从⾼到低排列)。如果检测到的细菌超过30种，则仅列出丰度最⾼的前30种。“群体水平%”是指这类细菌在数据库中有多少比例的狗狗身上被发现，通常用百分比来表示'
};

function generateDetectedGutMicrobiomeGenusTable(DetectedGutMicrobiomeGenusData) {

  let detectedGutMicrobiomeGenusTable = '';

  const top30Bacteria = DetectedGutMicrobiomeGenusData
    .map(
      (bacteria) => (
        {
          ...bacteria,
  ...
  top30Bacteria.forEach(
    (bacteria, index) => {  // 添加index参数
      ...
      detectedGutMicrobiomeGenusTable += `
  ...
  return detectedGutMicrobiomeGenusTable;
}

function generateDetectedGutMicrobiomeGenusHTML(DetectedGutMicrobiomeGenusData) {
  let detectedGutMicrobiomeGenusHTML = '';
  detectedGutMicrobiomeGenusHTML += `
    ...
    <table
      ...
      <tbody>
        ${generateDetectedGutMicrobiomeGenusTable(DetectedGutMicrobiomeGenusData)}
      </tbody>
    </table>
  `;
  return detectedGutMicrobiomeGenusHTML;
}

function loadDetectedGutMicrobiomeGenusHTML(SampleData) {
  // 生成肠道评估的 HTML
  // 将生成的 HTML 插入到 ID 为 "intestinalAssessment" 的 div 中
  document.getElementById(
    'detected-gut-microbiome-genus'
  ).innerHTML = generateDetectedGutMicrobiomeGenusHTML(SampleData['菌群构成-属']);
}

export {loadDetectedGutMicrobiomeGenusHTML};
